...
TGEMSBSGEMPlane::TGEMSBSGEMPlane()
...
TGEMSBSGEMPlane::TGEMSBSGEMPlane( const char *name, const char *desc,
			    ...
			    Bool_t /* required */)
{
  ...
  TGEMSBSGEMChamber* parent = (TGEMSBSGEMChamber*) GetParent();
  //Double_t d0;
  Double_t depth;
  if (parent != NULL)
    {
...
Double_t 
TGEMSBSGEMPlane::GetSAngle()   const
{
  return fSAngle;
}
...
void
TGEMSBSGEMPlane::LabToStrip (Double_t& x, Double_t& y, Double_t& z) const
{
  LabToPlane(x, y, z);
  PlaneToStrip(x, y);
...
void
TGEMSBSGEMPlane::SpecToStrip (Double_t& x, Double_t& y) const
{
  SpecToPlane(x, y);
  PlaneToStrip(x, y);
...
void
TGEMSBSGEMPlane::StripToSpec (Double_t& x, Double_t& y) const
{
  StripToPlane(x, y);
  PlaneToSpec(x, y);
  // Double_t temp = x;
...
void
TGEMSBSGEMPlane::StripToPlane (Double_t& x, Double_t& y) const
{
  Double_t temp = x;
  x = fCBS * x + fSBS * y;
  y = -fSBS * temp + fCBS * y;
...
void
TGEMSBSGEMPlane::StripToLab (Double_t& x, Double_t& y, Double_t& z) const
{
  StripToPlane(x, y);
  PlaneToLab(x, y, z);
...
Double_t TGEMSBSGEMPlane::StripNumtoStrip( Int_t strip )
{
...
Double_t TGEMSBSGEMPlane::StriptoProj( Double_t s )
{
...
Double_t TGEMSBSGEMPlane::StripNumtoProj( Int_t s ){
    // Gives coordinate in projection frame from strip number
    return StriptoProj( StripNumtoStrip(s) );
}

Double_t 
TGEMSBSGEMPlane::GetStripLowerEdge (UInt_t is) const {return (fSBeg + is * GetSPitch());}

Double_t 
TGEMSBSGEMPlane::GetStripUpperEdge (UInt_t is) const {return GetStripLowerEdge (is) + GetSPitch();}


Int_t
TGEMSBSGEMPlane::GetStripUnchecked( Double_t x ) const
{
...
Int_t
TGEMSBSGEMPlane::GetStripInRange( Double_t x ) const
{
  // Get strip number for given x-coordinate in strip frame
...
Int_t
TGEMSBSGEMPlane::GetStrip (Double_t x, Double_t yc) const
{
...
void 
TGEMSBSGEMPlane::Print( Option_t* opt ) const
{
...
void
TGEMSBSGEMPlane::SetRotations()
{
  // Calculate the rotations from the strip to the lab and vice versa.
  // This is very similar to what is made in TGEMSBSBox
