#ifndef __TGEMSBSGEMPLANE_H
#define __TGEMSBSGEMPLANE_H
...
// In the SBS geometry, a plane is a box (as defined it TGEMSBSBox class). 
// Refer to the comment in the header of TGEMSBSBox class for more info.

// In the code, TGEMSBSGEMPlane contains the information about the strips and such 
...
// TGEMSBSGEMPlane also inherits form THaSubDetector, which grants it all the functions from its class
...
class TGEMSBSGEMPlane : public THaSubDetector {
    public:
        TGEMSBSGEMPlane ();
	TGEMSBSGEMPlane(const char *name, const char *desc,
...
inline void
TGEMSBSGEMPlane::PlaneToStrip (Double_t& x, Double_t& y) const
{
  Double_t temp = x;
  x = fCBS * x - fSBS * y;
  y = fSBS * temp + fCBS * y;
