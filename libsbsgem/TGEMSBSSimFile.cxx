...
//   uses them to fill a TGEMSBSSimEvent object. A pointer to the event
//   object is available via GetEvBuffer() for use by the decoder.
//
/////////////////////////////////////////////////////////////////////

#include "TGEMSBSSimFile.h"
...
//-----------------------------------------------------------------------------
TGEMSBSSimFile::TGEMSBSSimFile(const char* filename, const char* description) :
...
//-----------------------------------------------------------------------------
TGEMSBSSimFile::TGEMSBSSimFile(const TGEMSBSSimFile &run)
...
//-----------------------------------------------------------------------------
TGEMSBSSimFile& TGEMSBSSimFile::operator=(const THaRunBase& rhs)
{
  if (this != &rhs) {
    THaRunBase::operator=(rhs);
    if( rhs.InheritsFrom("TGEMSBSSimFile") )
      fROOTFileName = static_cast<const TGEMSBSSimFile&>(rhs).fROOTFileName;
    fROOTFile = 0;
    fTree = 0;
    fEvent = 0;
    fNEntries = fEntry = 0;
...
Int_t TGEMSBSSimFile::Compare( const TObject* obj ) const
{
  // Compare two TGEMSBSSimFiles. They are different if either their number
  ...
  const TGEMSBSSimFile* rhsr = dynamic_cast<const TGEMSBSSimFile*>(rhs);
  if( !rhsr ) return 0;
...
//-----------------------------------------------------------------------------
Int_t TGEMSBSSimFile::ReadEvent()
{
...
const UInt_t *TGEMSBSSimFile::GetEvBuffer() const
#else
const  Int_t *TGEMSBSSimFile::GetEvBuffer() const
#endif
{
  if( !IsOpen() ) return 0;
...
//_____________________________________________________________________________
void TGEMSBSSimFile::Print( Option_t* opt ) const
{
