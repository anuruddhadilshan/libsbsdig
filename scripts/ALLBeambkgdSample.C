...
{
  cout << " This file uses root generated class all_deftree. " << endl
       ...
       << " you may generate file all_deftree.C and .h yourself " << endl
       << " by opening one of your g4sbs files and type: " <<
       << " T->MakeClass(\"all_deftree\") " << endl;
  ...
  TH1D* h1_SBSGEM_Edep_log_[5];
  
  for(int m = 0; m<5; m++){
    h1_SBSGEM_nhits_[m] = new TH1D(Form("h1_SBSGEM_nhits_%d",m), 
				  Form("SBS GEM plane %d energy deposit", m+1),
				  500, 0.0, 500);
    h1_SBSGEM_yVsx_[m] = new TH2D(Form("h1_SBSGEM_yVsx_%d",m), 
  				 Form("SBS GEM plane %d y vs x coordinate", m+1),
  				 202, -1.01, 1.01, 62, -0.31, 0.31);
    h1_SBSGEM_dyVsdx_[m] = new TH2D(Form("h1_SBSGEM_dyVsdx_%d",m), 
				   Form("SBS GEM plane %d dy vs dx coordinate", m+1),
				   100, -0.1, 0.1, 100, -0.1, 0.1);
    h1_SBSGEM_Edep_[m] = new TH1D(Form("h1_SBSGEM_Edep_%d",m), 
  				 Form("SBS GEM plane %d energy deposit;MeV", m+1),
  				 100, 0.0, 1.e-1);
    h1_SBSGEM_Edep_log_[m] = new TH1D(Form("h1_SBSGEM_Edep_log_%d",m), 
				     Form("SBS GEM plane %d energy deposit;MeV", m+1),
				     110, edepbins_log10);
  }
  ...
  memset(nhits_BBGEM, 0, 5*sizeof(int));
  ...
  while (( chEl=(TChainElement*)next() )) {
    ...
    // root gmn_beam_bkgd_0.root
    // root [0] T->MakeClass("gmn_deftree");
    all_deftree *T1 = new all_deftree(C1);
	...
	for(int i = 0; i<T1->Earm_BBGEM_hit_nhits; i++){
      ...
      // fill SuperBigBite GEMs energy deposit, position, and drift exit-entrance position difference histograms, and increment number of hits
      //cout << "SBSGEM: " << T1->Harm_SBSGEM_hit_nhits << endl;
      if(T1->Harm_SBSGEM_hit_nhits){
	for(int i = 0; i<T1->Harm_SBSGEM_hit_nhits; i++){
	  nplane = T1->Harm_SBSGEM_hit_plane->at(i)-1;
	  
	  nhits_SBSGEM[nplane]++;
	  // 
	  
	  // //cout << T1->Harm_SBSGEM_hit_tx->at(i) << " " << T1->Harm_SBSGEM_hit_ty->at(i) << endl;
	  
	  h1_SBSGEM_yVsx_[nplane]->Fill(T1->Harm_SBSGEM_hit_xin->at(i), 
	   			       T1->Harm_SBSGEM_hit_yin->at(i));
	  h1_SBSGEM_dyVsdx_[nplane]->Fill(T1->Harm_SBSGEM_hit_xout->at(i)-T1->Harm_SBSGEM_hit_xin->at(i), 
					 T1->Harm_SBSGEM_hit_yout->at(i)-T1->Harm_SBSGEM_hit_yin->at(i));
	  
	  h1_SBSGEM_Edep_[nplane]->Fill(T1->Harm_SBSGEM_hit_edep->at(i)*1.e3);
	  h1_SBSGEM_Edep_log_[nplane]->Fill(T1->Harm_SBSGEM_hit_edep->at(i)*1.e3);
    ...
    if(FileCounter%1==0){
      ...
      //if(FileCounter%15==0){
      for(int j = 0; j<5; j++){
	h1_BBGEM_nhits_[j]->Fill(nhits_BBGEM[j]);
      }
      memset(nhits_BBGEM, 0, 5*sizeof(int));

      for(int j = 0; j<5; j++){
	h1_SBSGEM_nhits_[j]->Fill(nhits_SBSGEM[j]);
      }
      memset(nhits_SBSGEM, 0, 5*sizeof(int));
    }
