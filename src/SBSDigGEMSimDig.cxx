...
#define fPulseShapeTau0 56.   // [ns] GEM model 0 = 50. in SiD model
...
SBSDigGEMSimDig::SBSDigGEMSimDig(int nchambers, double* trigoffset, double zsup_thr, std::vector<Double_t>* commonmode_array) : fNChambers(nchambers), fZeroSup(zsup_thr) 
{
  if(fZeroSup>0)fDoZeroSup = true;
  if(commonmode_array){
    fDoCommonMode = true;
    fCommonModeArray = commonmode_array;
  }else{
    fDoCommonMode = false;
    fCommonModeArray = 0;
  }
...
/*
Int_t
SBSDigGEMSimDig::AdditiveDigitize (TRandom3* R)
{
  ...
  for (UInt_t ih = 0; ih < nh; ++ih) {  
	...
	delete fdh[j]; //Danning: This line is newly added, because one needs to release memory fdh points if one really wants to points fdh to NULL in the following line
	               //         On the other hand, fdh seems to be specific to a certain GEM thus shouldn't be member of TGEMSBSSimDigitization and should
...
Double_t TGEMSBSSimDigitization::fCrossFactor = 0.;
Double_t TGEMSBSSimDigitization::fCrossSigma = 0.;


// Auxiliary class

TGEMSBSDigitizedPlane::TGEMSBSDigitizedPlane (UShort_t nstrip,
					UShort_t nsample,
					Int_t threshold )
...
TGEMSBSDigitizedPlane::~TGEMSBSDigitizedPlane()
{
  delete[] fType;
  delete[] fCharge;
  delete[] fTime;
...
UShort_t
TGEMSBSDigitizedPlane::Threshold( Int_t thr )
{
...
TGEMSBSSimDigitization::TGEMSBSSimDigitization( const TGEMSBSSpec& spect,
  ...
  : THaAnalysisObject(name, "GEM simulation digitizer"),
    fDoMapSector(false), fSignalSector(0), fDP(0), fdh(0), fNChambers(0), fNROPlanes(0),
...
void TGEMSBSSimDigitization::DeleteObjects()
{
  ...
  delete[] fDP;       fDP = 0;
  delete[] fdh;       fdh = 0;
  delete[] fNROPlanes;  fNROPlanes = 0;
...
void
TGEMSBSSimDigitization::Initialize(const TGEMSBSSpec& spect)
{
  ...
  fNROPlanes = new UInt_t[fNChambers];
  for (UInt_t ic = 0; ic < fNChambers; ++ic)
    {
      fNROPlanes[ic] = spect.GetChamber(ic).GetNPlanes();
      fDP[ic] = new TGEMSBSDigitizedPlane*[fNROPlanes[ic]];
  ...
  Double_t f = ( 2 * fAvalancheFiducialBand * 0.1 ) / pitch + 6 ;// 0.1: fRSMax; 6: track slope
...
//Int_t TGEMSBSSimDigitization::Digitize (const TGEMSBSGEMSimHitData& gdata, const TGEMSBSSpec& spect)
...
TGEMSBSGEMHit **
TGEMSBSSimDigitization::AvaModel(const Int_t ic,
...
#define DBG_AVA 0
  ...
  for (UInt_t ipl = 0; ipl < fNROPlanes[ic]; ++ipl){
#if DBG_AVA > 0
    ...
    const TGEMSBSGEMPlane& pl = chamber.GetPlane(ipl);
...
// 
	...
	Double_t pulse =
	  TGEMSBSSimAuxi::PulseShape (fEleSamplingPeriod * b - t0,
				   us,
				   fPulseShapeTau0,
				   fPulseShapeTau1 );

	Short_t dadc = TGEMSBSSimAuxi::ADCConvert( pulse,
						fADCoffset,
						fADCgain,
						fADCbits );
#if DBG_AVA > 0
...
void
TGEMSBSSimDigitization::PrintSamples() const
{
				    ...
				    Double_t t0 )
{
  ...
  const TGEMSBSGEMChamber& ch = spect.GetChamber(igem);
  ...
  // cout << " TGEMSBSSimDigitization.cxx:  hit in plane " << clust.fPlane << " (clust.fHitPos):  " << endl;
  // clust.fHitpos.Print();
  ...
  for (UInt_t j = 0; j < 2; j++) {
    ...
    const TGEMSBSGEMPlane& pl = ch.GetPlane(j);
...
{
  return; // Let's not do this here. Let SimDigitizer.cxx do all the work
...
void
TGEMSBSSimDigitization::WriteTree () const
{
...
void
TGEMSBSSimDigitization::CloseTree () const
{
