...
namespace Decoder {
  ...
  Module::TypeIter_t TSBSSimMPD::fgThisType =
    DoRegister( ModuleType( "Decoder::TSBSSimMPD" , 53561 ));

  TSBSSimMPD::TSBSSimMPD()
  {
    //fadc_data.resize(NADCCHAN);
  }

  TSBSSimMPD::TSBSSimMPD(Int_t crate, Int_t slot)
  {
    //fadc_data.resize(NADCCHAN);
    IsInit = kFALSE;
    Init();
  ...
  /*
  Bool_t TSBSSimMPD::HasCapability(Decoder::EModuleType type) {
  ...
  void TSBSSimMPD::ClearDataVectors() {
  ...
  void TSBSSimMPD::Clear( const Option_t* opt) {
    // Clear event-by-event data
    ClearDataVectors();
  }

  void TSBSSimMPD::Init() {
    Clear();
    IsInit = kTRUE;
    fName = "SBSSimMPD (Simple JLab Flash ADC Simulated Module)";
  }

  void TSBSSimMPD::CheckDecoderStatus() const {
  }

  Int_t TSBSSimMPD::LoadSlot(THaSlotData *sldat, const UInt_t *evbuffer,
      const UInt_t *pstop) {
    Clear();
  ...
  Int_t TSBSSimMPD::LoadSlot(THaSlotData *sldat, const UInt_t *evbuffer,
    ...
    //return TSBSSimMPD::LoadSlot(sldat,evbuffer,len);
  }

}

ClassImp(Decoder::TSBSSimMPD)
